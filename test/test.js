...
contract('DVideo', ([deployer, author]) => {
  ...
  describe('videos', async () => {
    ...
    it("delete videos", async () => {
      
      await dvideo.deleteVideo(videoCount, { from: author });
      
      const deleted = await dvideo.videos(videoCount);
      
      assert.equal(deleted.id.toNumber(), 0, 'id is correct')
      assert.equal(deleted.hash, "", 'Hash is correct')
      assert.equal(deleted.title, "", 'Title is correct')
    }); 
  })
})